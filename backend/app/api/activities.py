...
def get_activities(db: Session = Depends(get_db), current_user: users_schemas.User = Depends(get_current_user)):
...
def get_activity(activity_id: int, db: Session = Depends(get_db), current_user: users_schemas.User = Depends(get_current_user)):
...
def create_activity(activity: schemas.ActivityCreate, db: Session = Depends(get_db), current_user: users_schemas.User = Depends(get_current_user)):
...
def update_activity(activity_id: int, activity: schemas.ActivityUpdate, db: Session = Depends(get_db), current_user: users_schemas.User = Depends(get_current_user)):
    return crud.update_activity(db, activity_id, activity)

# Route to delete an activity
@router.delete("/activities/{activity_id}")
def delete_activity(activity_id: int, db: Session = Depends(get_db), current_user: users_schemas.User = Depends(get_current_user)):
